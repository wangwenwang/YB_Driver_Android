...
import com.example.administrator.ybdriver.utils.ExceptionHandler;
 ...
 * 管理信息Activity
 */
public class ManageHorizontalChartActivity extends BaseFragmentActivity implements OnChartValueSelectedListener,AsyncHttpCallback {
...
//    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_manage_horizontalchart);
        initview();
        //  initLableData();
        //  initEntries();
        //  initshow();
    ...
    private void initBarData(int qty) {
       ViewGroup.LayoutParams layoutParams=mbarchart.getLayoutParams();
        if (barEntries.size()>7){
            ...
            layoutParams.height=UIUtils.dip2px(350);
        }
...
//        });
    ...
    private void  initPie1Data(int totalqty){
        ...
        //            xPieVals.add(new PieEntry(lcBusinessf*100f,"发货:"+lcBusinesslist.get(i).getQtyTotal(),i));
                    xPieVals.add(new PieEntry(lcBusinessf*100f,lcBusinesslist.get(i).getTms_fllet_name()+" 数量:"+lcBusinesslist.get(i).getQtyTotal()+" 占比:"+ FloatArithUtil.round(lcBusinessf*100f,1)+"%",i));
        ...
        //    xPieVals.add(new PieEntry(lastf*100f,"发货:"+lcBusinesslist.get(lcBusinesslist.size()-1).getQtyTotal(),lcBusinesslist.size()-1));
            xPieVals.add(new PieEntry(lastf*100f,lcBusinesslist.get(lcBusinesslist.size()-1).getTms_fllet_name()+" 数量:"+lcBusinesslist.get(lcBusinesslist.size()-1).getQtyTotal()+" 占比:"+FloatArithUtil.round(lastf*100f,1)+"%",lcBusinesslist.size()-1));
            ...
            mpiechart1.setCenterText("物流商发货占比图");
          ...
          //  mpiechart1.setExtraOffsets(5, 5, 5, 5);
            ...
            mpiechart1.setDrawEntryLabels(false);//取消显示图例注解在圆饼上
            //设置右上角注解条目样式
            ...
            l.setEnabled(false);
            legendAdapter = new PieLegendAdapter(ManageHorizontalChartActivity.this, Arrays.asList(colors));
...
//            l.setYOffset(0f);
    ...
    private void initview() {
        ...
        titelView.setText(getString(R.string.wl_manage_info));
    ...
    private void getChartData() {
        ...
        mClient.sendRequest(Constants.URL.GetDateCestbonCount, params, TAG_GetCestbonCount);
    }
        ...
        if (request_tag.equals(TAG_GetCestbonCount)){
            ...
            if(lcBusinesslist==null){
                tvStartTime.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        new SlideDateTimePicker.Builder(getSupportFragmentManager())
                                .setListener(new DateHandler(tvStartTime.getId()))
                ...
                tvEndTime.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        new SlideDateTimePicker.Builder(getSupportFragmentManager())
                                .setListener(new DateHandler(tvEndTime.getId()))
                ...
                for (int i=0;i<lcBusinesslist.size()-1;i++){
                    for (int j=0;j<lcBusinesslist.size()-1-i;j++){
                        if (lcBusinesslist.get(j).getQtyTotal()>lcBusinesslist.get(j+1).getQtyTotal()){
                            LCBusiness temp=lcBusinesslist.get(j);
    ...
    private void setData(List<LCBusiness> lcBusinesslist) {
        ...
        for (int i=0;i<lcBusinesslist.size();i++){
            ...
            if (lcBusiness.getQtyTotal()>0) {
                labels.add(lcBusiness.getQtyTotal()+"/"+lcBusiness.getTms_fllet_name());
